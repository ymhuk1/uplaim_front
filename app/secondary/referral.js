                    ...
                    <View style={styles.containerView}>
                        ...
                        <View style={styles.topContainer}>
                            <View style={styles.topMini}>
                                ...
                                <Text style={styles.topMiniText}>{clientData && clientData.balance} ₽</Text>
                            </View>
                            <View style={styles.topMini}>
                                <View style={styles.topMiniHead}>
                                    <Image
                                ...
                                <Text style={styles.topMiniText}>{earning ? earning : '0'} ₽</Text>
                            </View>
                        </View>
                        {clientData && clientData.tariff.name === 'Free' ? (
                            <View style={styles.tariffFreeContainer}>
                                <Text style={styles.tariffFreeHeader}>Для получения вознаграждения подключите тариф</Text>
                                <View style={styles.tariffFreeInfo}>
                                    <View style={styles.tariffFreeLeft}>
                                        <Text style={[styles.tariffFreeName, {backgroundColor: 'red'}]}>Pro</Text>
                                        <Text style={[styles.tariffFreeName, {backgroundColor: 'blue'}]}>Premium</Text>
                                        <Text style={[styles.tariffFreeName, {backgroundColor: 'orange'}]}>VIP</Text>
                                    </View>
                                    <View style={styles.tariffFreeRight}>
                                        <Text style={styles.tariffFreeInfoText}>и зарабатывайте 150 ₽ в день </Text>
                                        <Text style={styles.tariffFreeInfoText}>и зарабатывайте 256 ₽ в день </Text>
                                        <Text style={styles.tariffFreeInfoText}>и зарабатывайте 369 ₽ в день </Text>
                                    </View>
                                </View>
                                <Text style={styles.tariffFreeDopText}>Вознаграждение также зависит от количества друзей</Text>
                            </View>
                        ) : (
                            <View>
                            {referral && referral.map((item) => (
                                <View>
                                    <View style={styles.levelContainer}>
                                        ...
                                        <View style={styles.levelTopContainer}>
                                            <View style={styles.levelLeft}>
                                                <View style={styles.levelLeftContainer}>
                                                    <View style={styles.levelLeftTextContainer}>
                                                        <Text style={styles.levelLeftNameText}>{`${item.level} уровень`}</Text>
                                                        <Text style={styles.levelLeftPeopleText}>{`${item.quantity} человек`}</Text>
                                                    </View>
                                                    <Image
                                                        contentFit="contain"
                                                        contentPosition={"center"}
                                            ...
                                            <View style={styles.levelRight}>
                                                <Text style={styles.levelLeftDiscountText}>{`${item.reward}%`}</Text>
                                                <Text style={styles.levelLeftDopText}>с уровня</Text>
                                            </View>
                                        </View>
                                        <View style={styles.levelBottomContainer}>
                                            <View style={styles.tariffsContainer}>
                                                {item.tariff_counts.map((tariff) => (
                                                    <View key={tariff.tariff} style={[styles.tariffContainer, {backgroundColor: tariff.color}]}>
                                                        <Text style={styles.tariffNameText}>{tariff.tariff} {tariff.count}</Text>
                                                    </View>
                                                ))}
                                            </View>
                                            {item.level === '1' ? (
                                                <TouchableOpacity onPress={() => {setOpenPeople(!openPeople)}}>
                                                    <Image
                                                        style={[!openPeople ? {transform: [{ rotate: '180deg' }]} : '' ]}
                                                        contentFit="contain"
                                                        contentPosition="center"
                                                        transition={1000}
                                            ...
                                            {item.level === '1' ? (
                                                <View style={styles.peoplesContainer}>
                                                    {item.people.map((people, index) => (
                                                        <View key={people.tariff_name}
                                                              style={[styles.peopleContainer, index % 3 === 2 ? styles.lastInRow : null]}>
                                                            <Text style={styles.peopleName}>{people.name} {people.surname ? people.surname.substring(0, 1) : ''}.</Text>
                                                            <View style={[styles.peopleTariffContainer, {backgroundColor: people.tariff_color}]}>
                                                                <Text style={styles.peopleTariffName}>{people.tariff_name}</Text>
                                                            </View>
