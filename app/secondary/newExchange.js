...
export default function NewCreateExchangeScreen() {
  ...
  const [selectedItem, setSelectedItem] = useState(
    deal === "edit" || deal === "offer" ? existCompanyId : null
  );
  ...
  const [selectBallsTag, setSelectBallsTag] = useState([
    { name: "Баллы в компании" },
    { name: "Баллы в категории" },
  ]);
  const [companyOrCategory, setCompanyOrCategory] = useState(null);
  const [selectVariantTag, setSelectVariantTag] = useState(
    "Несколько вариантов"
  );
  ...
  const [takerFilterExchanges, setTakerFilterExchanges] =
    useState("city=Оренбург");
  ...
  useEffect(() => {
    ...
    if (deal === "edit") {
      ...
      const parsedCompanyTaker = existCompanyTaker
        ? JSON.parse(existCompanyTaker)
        : JSON.parse(existCategoryTaker);
      const parsedCompanyTakerCategory = existCategoryTaker
        ? JSON.parse(existCategoryTaker)
        : JSON.parse(existCompanyTaker);

      if (parsedCompanyTakerCategory > parsedCompanyTaker) {
        setSelectedTakerItems(
          Array.isArray(parsedCompanyTakerCategory)
            ? parsedCompanyTakerCategory
            : parsedCompanyTaker
        );
      } else {
        setSelectedTakerItems(
          Array.isArray(parsedCompanyTaker)
            ? parsedCompanyTaker
            : parsedCompanyTakerCategory
        );
      }
  ...
  const fetchData = async () => {
    try {
      ...
      if (userDataStr) {
        ...
        if (!cityResponse.ok) {
          throw new Error(
            `Failed to fetch city. Status: ${cityResponse.status}`
          );
        }
  ...
  const renderItems = (items, isTaker = false) => (
    <View>
      {items.map((item, index) => {
        const shouldDisplay =
          (isTaker &&
            (!hideOtherItemsTaker || selectedTakerItems.includes(item.id))) ||
          (!isTaker && (!hideOtherItems || selectedItem === item.company_id));

        const shouldDisplayEdit =
          (deal === "edit" || deal === "offer") &&
          item.company_id === existCompanyId;
          ...
          return (
            <TouchableOpacity
              ...
              style={[
                styles.itemGive,
                isTaker &&
                  selectedTakerItems.includes(item.id) &&
                  styles.itemGiveSelected,
                !isTaker &&
                  selectedItem === item.company_id &&
                  styles.itemGiveSelected,
              ]}
            >
              <Image
              ...
              <View style={styles.containerText}>
                <Text style={styles.companyGiveText}>
                  {isTaker ? item.name : item.company_name}
                </Text>
                {!isTaker && (
                  <View style={styles.containerBalls}>
                    <Image
                      ...
                      height={12}
                    />
                    <Text style={styles.ballsGiveText}>
                      {item.balls} {getBallsText(item.balls)}
                    </Text>
                  </View>
                )}
              </View>
  ...
  const handleValueChange = (selectedValue, tagName) => {
    ...
    if (tagName === "takerCityTags") {
      if (takerTagsCategory === "Все") {
        ...
        setTakerFilterExchanges(
          `city=${selectedValue}&category=${takerTagsCategory}`
        );
      }
    ...
    if (tagName === "takerCategoryTags") {
      if (takerTagsCity === "Все") {
        ...
        setTakerFilterExchanges(
          `city=${takerTagsCity}&category=${selectedValue}`
        );
      }
              ...
              <View>
                ...
                <View style={styles.tagContainer}>
                  <NewTagComponent
                    ...
                    valueOnChange={(selectedValue) =>
                      handleValueChange(selectedValue, "cityTags")
                    }
                  />
                  <NewTagComponent
                    tag={categoryTags}
                    valueOnChange={(selectedValue) =>
                      handleValueChange(selectedValue, "categoryTags")
                    }
                  />
                </View>
                {ballsData && ballsData.length !== 0 ? (
                  <View>{renderItems(ballsData, false)}</View>
                ) : (
                  <View>
                    <Text style={styles.dopText}>
                      У вас нет доступных компаний
                    </Text>
                  </View>
                )}
                ...
                <View style={styles.tagContainer}>
                  <NewTagComponent
                    tag={cityTags}
                    valueOnChange={(selectedValue) =>
                      handleValueChange(selectedValue, "takerCityTags")
                    }
                  />
                  <NewTagComponent
                    tag={selectBallsTag}
                    valueOnChange={(selectedValue) =>
                      handleValueChange(selectedValue, "selectBallsTag")
                    }
                  />
                  ...
                  <NewTagComponent
                    tag={categoryTags}
                    valueOnChange={(selectedValue) =>
                      handleValueChange(selectedValue, "takerCategoryTags")
                    }
                  />
                </View>
            ...
            {typeDeal === "buy" && (
              <View>
                ...
                <View style={styles.tagContainer}>
                  ...
                  />
                  <NewTagComponent
                    tag={selectVariantTag}
                    valueOnChange={(selectedValue) =>
                      handleValueChange(selectedValue, "selectVariantTag")
                    }
                  />
                  <NewTagComponent
                    tag={categoryTags}
