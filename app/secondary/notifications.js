...
import {elemBackgroundColor, fuchsia, textColor3, textColor4, textPrimaryColor} from "../../components/ColorsComponent";
...
export default function Notifications() {
    ...
    const fetchData = (id, is_read, date, type_notify) => {
        ...
        const url = `${apiBaseUrl}/api/notifications?client_id=${id}${is_read ? `&is_read=${is_read}` : ''}${date ? `&date=${encodedDate}` : ''}${type_notify ? `&type_notify=${type_notify}` : ''}`;
        console.log('URL:', url);
                ...
                <ImageBackground source={require('../../assets/background.png')} contentFit="cover">
                    <View style={styles.containerView}>
                        ...
                        {notificationsGroup?.length > 0 ? (
                            <View>
                                {Object.entries(groupNotificationsByTypeAndDate()).map(([dateKey, notificationsByDate]) => (
                                    <View key={dateKey}>
                                        <Text style={styles.dateText}>{ isToday(new UTCDate(dateKey)) ? "Сегодня" : format(new Date(dateKey), 'dd MMMM', {locale: ru}) }</Text>
                                        {Object.entries(notificationsByDate).map(([type, notifications]) => {
                                            ...
                                            return (
                                                <TouchableOpacity key={type} onPress={() => {
                                                    ...
                                                    fetchData(clientId, true, notifications[0].created_on, notifications[0].type)
                                                }}>
                                                    <View style={styles.containerGroupNotify}>
                                                        <View style={styles.containerGroupNotifyLeft}>
                                                            <View style={styles.groupNotifyLeftIcon}>
                                                                <Image
                                                                    ...
                                                                    transition={1000}
                                                                    source={getNotificationTypeLabel(type).iconSource}
                                                                    width={24}
                                                                    height={24}
                                                                />
                                                            </View>
                                                        ...
                                                        <View style={styles.containerGroupNotifyRight}>
                                                            ...
                                                            {unreadNotifications.length > 0 && (
                                                                <View style={styles.groupNotifyCountContainer}>
                                                                    <Text style={styles.groupNotifyCount}>{unreadNotifications.length}</Text>
                                                                </View>
                                                            )}
                                        ...
                                        <Modal
                                            ...
                                            <View style={styles.centeredView}>
                                                <View style={styles.modalContent}>
                                                    <Text style={styles.modalHeader}>{getNotificationTypeLabel(selectNotifications ? selectNotifications[0].type : '').label}</Text>
                                                    { selectNotifications && selectNotifications.map((notification, index) => (
                                                        <View key={index} style={styles.modalItem}>
                                                            ...
                                                            <View style={styles.modalText}>
                                                                <Text style={styles.modalTextHeader}>{notification.title}</Text>
                                                                <Text style={styles.modalTextDop}>{notification.description}</Text>
                                                            </View>
                                                        </View>
                                                    ))}
                                                </View>
                            ...
                            <View style={styles.containerEmptyNotify}>
                                <Text style={styles.emptyNotify}>У вас пока нет уведомлений</Text>
                            </View>
                        )}
