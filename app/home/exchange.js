...
export default function ExchangeScreen() {
  ...
  const [filterExchanges, setFilterExchanges] = useState(
    "available=true&city=Оренбург"
  );
  ...
  const fetchDataFilter = async (id) => {
    try {
      ...
      if (!exchangesResponse.ok) {
        throw new Error(
          `Failed to fetch available exchanges. Status: ${exchangesResponse.status}`
        );
      }
      const exchangesData = await exchangesResponse.json();
      ...
      if (!availableExchangesResponse.ok) {
        throw new Error(
          `Failed to fetch available exchanges. Status: ${availableExchangesResponse.status}`
        );
      }
  ...
  useEffect(() => {
    if (exchanges && availableExchanges) {
      const availableIds = new Set(
        availableExchanges.map((exchange) => exchange.id)
      );
    ...
    <View>
          ...
          <View style={styles.containerView}>
            ...
            <View>
              <InfoModalComponent
                ...
                titleText={
                  statusExchange === "update"
                    ? "Сделка изменена"
                    : "Сделка успешно создана"
                }
                titlePrimaryButton={"Хорошо"}
                onPress={() => setModalVisible(false)}
                onPressSecondary={() => setModalVisible(false)}
              />
            </View>
            <View style={styles.tagContainer}>
              <NewTagComponent
                ...
                valueOnChange={(selectedValue) =>
                  handleValueChange(selectedValue, "availableTags")
                }
              />
              <NewTagComponent
                tag={cityTags}
      ...
      <View style={styles.button}>
        <NewButtonComponent
          ...
          height={48}
          fontSize={18}
          onPress={() => router.push("/secondary/newExchange")}
        />
      </View>
    </View>
  );
