...
export default function SmsScreen() {
    ...
    const handleVerifyCode = async () => {
        ...
        try {
            ...
            console.log('Отправляю запрос на верификацию SMS-кода с данными:', JSON.stringify(requestBody));


            const response = await fetch('https://admin.saveup.pro/api/verify-sms-code', {
                method: 'POST',
                headers: {
            ...
            if (responseData.message === 'SMS code verified') {
                ...
                Alert.alert('Ошибка', 'SMS-код не верифицирован. Пожалуйста, проверьте код и попробуйте снова.');
            }
        } catch (error) {
            console.error('Ошибка при отправке запроса на верификацию SMS-кода:', error);
    ...
    const sendSmsRequest = async () => {
        try {
            ...
            console.log('Sending SMS request with data:', JSON.stringify(requestBody));

            const response = await fetch('https://admin.saveup.pro/api/send_phone_number', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Accept': '*/*',
            ...
            console.error('Ошибка при отправке запроса на повторную отправку SMS:', error);
            Alert.alert('Ошибка', 'Произошла ошибка при отправке запроса на сервер.');
        }
    };
            ...
            >
                <View style={styles.containerView}>
                    ...
                    <View style={styles.textContainer2}>
                        <Text style={styles.text2}>Мы выслали СМС-код на номер +{phoneNumber}</Text>
                    </View>
                    ...
                    <View style={styles.button}>
                        <NewButtonComponent title={'Подтвердить'} filled={true} height={48} fontSize={18} onPress={handleVerifyCode}/>
