...
export default function CreateExchangeModalComponent({visible, onRequestClose, selectedTakerItems, companyId, companiesData, ballsData, apiBaseUrl, typeDeal, edit, companyOrCategory}) {
    ...
    const { deal, existGiveBalls, existGetBalls, existExchangeId,
        existGiveSaveup, existGetSaveup, existGiveCash,
        existGetCash, existCompanyTaker, existTypeDeal, existHolderId } = useGlobalSearchParams()
        ...
        try {
            ...
            const response = await fetch(url, {
                ...
                body: JSON.stringify({
                    ...
                    get_saveup: (typeDeal === "exchange_sell" || typeDeal === 'sell') && isEnabled ? saveupCash : null,
                    give_cash: typeDeal === "buy" && !isEnabled ? saveupCash : null,
                    get_cash: (typeDeal === "exchange_sell" || typeDeal === 'sell') && !isEnabled ? saveupCash : null,
                    taker_companies: (companyOrCategory === 'Баллы в компании' || companyOrCategory === null) ? selectedTakerItems : [],
                    taker_categories:  companyOrCategory === 'Баллы в категории' ? selectedTakerItems : [],
            ...
            if (response.ok) {
                ...
                router.push(({ pathname: "/home/exchange",  params: { statusExchange: "create" }  }))
            } else {
    ...
    const updateDeal = async () => {
        try {
            ...
            if (response.ok) {
                ...
                router.push(({ pathname: "/home/exchange",  params: { statusExchange: "update" }  }))
    ...
    return (
        <Modal
            ...
            <View style={styles.createExchangeModal}>
                        ...
                        <View style={styles.modalContainer}>
                            {typeDeal === 'exchange' &&
                                <View style={styles.modalContainerGive}>
                                            ...
                                            <View style={styles.modalContainerTextAll}>
                                                <View style={styles.modalContainerCompanyText}>
                                                    ...
                                                    <Image
                                                        ...
                                                        height={33}
                                                    />
                                                    <Text style={styles.modalCompanyName}>{filteredBall.company_name}</Text>
                                                </View>
                                            </View>
                                            <View style={styles.modalContainerTextBallsGive}>
                                                <Text style={styles.modalTextBallsGive}>Количество баллов</Text>
                                            </View>
                                            <NewInputComponent value={giveBalls} maxLength={String(filteredBall.balls).length}
                                                               onChangeText={(value) => {
                                                                   const newValue = value.replace(/[^0-9]/g, '');
                                                                   const newNumber = Math.min(Number(newValue), Number(filteredBall.balls));
                                    ...
                                    <Image
                                        style={[styles.containerIconMiddleExchange, {transform: [{rotate: '90deg'}],} ]}
                                        contentFit="contain"
                                        contentPosition="center"
                                        ...
                                        height={32}/>
                                    <View style={styles.modalContainerTextAll}>
                                        {selectedTakerItems && selectedTakerItems.map((selectedItemId, index) => {
                                            const selectedCompany = companiesData.find(company => company.id === selectedItemId);
                                    ...
                                    <NewInputComponent value={getBalls} onChangeText={setGetBalls} maxLength={10} placeholder={'Введите'}/>
                                    <View style={styles.modalCheckbox}>
                                        <NewCheckboxComponent isChecked={isChecked} onToggle={setIsChecked} title={"Возможность частичного обмена"} />
                                    </View>
                                </View>
                            }
                            ...
                            {typeDeal === 'exchange_sell' &&
                                <View style={styles.modalContainerGive}>
                                                    ...
                                                    <View key={index}>
                                                        <View style={styles.modalContainerCompanyText}>
                                                            ...
                                                            <Image
                                                                ...
                                                                contentFit="contain"
                                                                contentPosition="center"
                                                                source={ apiBaseUrl + ((deal === 'edit' || deal === 'offer') ? selectedCompany.photo : selectedCompany.photo) }
                                                                width={33}
                                                                height={33}
                                                            />
                                                            {(deal === 'edit' || deal === 'offer') ? (
                                    ...
                                    <SwitchComponent isEnabled={isEnabled} onToggle={handleSwitchToggle} />
                                    <NewInputComponent value={saveupCash} onChangeText={setSaveupCash} maxLength={10} placeholder={'Введите'}/>
                                </View>
                            }
                            {typeDeal === 'buy' &&
                                <View style={styles.modalContainerGive}>
                                    ...
                                    <SwitchComponent isEnabled={isEnabled} onToggle={handleSwitchToggle} />
                                    <NewInputComponent value={saveupCash} onChangeText={setSaveupCash}/>
                                    <Text style={[styles.modalDopText, styles.modalDopTextMore]}>баллы компаний</Text>
                            ...
                            {typeDeal === 'sell' &&
                                <View style={styles.modalContainerGive}>
                                    {filteredBall &&
                                        <View style={styles.modalContainerGive}>
                                            <Text style={[styles.modalDopText, styles.modalDopTextMore]}>Продать баллы</Text>
                                            <View style={styles.modalContainerTextAll}>
                            ...
                            <View>
                                <Text style={styles.modalTextCommission}>Комиссия за сделку составит 10%, согласно вашему тарифному плану. Вы получите 280 баллов на свой баланс</Text>
                            </View>
                            <View style={styles.modalCreateButton}>
                                {(deal === 'edit') ? (
                                    <NewButtonComponent title={'Сохранить сделку'} filled={true} height={48} fontSize={18} onPress={updateDeal}/>
                                ) : (
                                    <View>
                                        {(deal === 'offer') ? (
                                            <NewButtonComponent title={'Встречная сделка'} filled={true} height={48} fontSize={18} onPress={createDeal}/>
                                        ) : (
                                            <NewButtonComponent title={'Создать сделку'} filled={true} height={48} fontSize={18} onPress={createDeal}/>
                                        )}
                                    </View>
                                    )}
                            </View>
                ...
                <TouchableOpacity style={styles.closeButton} onPress={(deal === 'edit' || deal === 'offer') ? () => router.push('home/exchange') : onRequestClose}>
                    <Image
                        contentFit="contain"
                        contentPosition="center"
