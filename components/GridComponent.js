...
import {
    elemBackgroundColor,
    elemGradientColors, elemGradientColors2,
    textColor3,
    textDisabledColor,
    textPrimaryColor
...
const GridComponent = ({ data, itemsPerSlide, itemHeight, myCompany, partners, coupon, companySlider, addCompany, companyTariffs, qrcode, hide }) => {
    ...
    return (
        ...
        <View style={styles.container}>
                        ...
                        <View style={styles.elemContainer}>
                            ...
                            <View style={styles.ballsContainer}>
                                ...
                                <Image
                                    ...
                                    height={14}
                                    />
                                <Text style={styles.balls}>{item.client_balls !== undefined ? item.client_balls : 0} {getBallsText(item.client_balls)}</Text>
                            </View>
                        </View>
                    )}
                    {partners && (
                        <TouchableOpacity onPress={() => router.push({ pathname: "/home/companyList",  params: { id: item.id }  })}
                        >
                                <View style={styles.partnersContainer} key={item.id}>
                                    ...
                                    <View style={[styles.borderPartners, {borderColor: item.color}]}></View>
                                    <View>
                                            ...
                                            <View style={styles.topContainer}>
                                                ...
                                                <View style={styles.infoTopContainer}>
                                                    <View style={styles.textTopContainer}>
                                                        <Text style={styles.textTop}>{item.company.name}</Text>
                                                    </View>
                                                    {/*<TagComponent tags={item.company.tags}/>*/}
                                                </View>
                                            </View>
                                            <Text style={styles.description}>{itemState.description}</Text>
                                            <Text style={styles.descriptionSmall}>Для активации промокода нажмите на кнопку Получить. С вас спишется 10UP за активацию промокода.</Text>
                                            {hideButtonGet && (
                                                <NewButtonComponent title={'Получить'} filled={true} height={48} fontSize={18} onPress={() => {setOpenQRCode(!openQRCode); setHideButtonGet(!hideButtonGet)}}/>
                                            )}
                                            {openQRCode && (
                                                <View style={styles.codeContainer}>
                                                    <QRCodeComponent data="532 783" size={280} logoSize={88} style={styles.codeStyle}/>
                                                </View>
                                            )}
                                        </View>
                                    </View>
                    ...
                    {companySlider && (
                        ...
                        <View style={styles.companySliderContainer}>
                            ...
                            <View style={styles.companyDescContainer}>
                                <Text style={styles.companyDesc}>{item.short_description ? truncateText(item.short_description, 55) : ''}</Text>
                            </View>
                        </View>
                    )}
                    {qrcode && (
                        <View style={[styles.elemContainerQR, hide === true && {opacity: 0.3}  ]}>
                            <View style={styles.itemActivity}>
                                <View style={[styles.backActivity, {backgroundColor: `${item.category.color}20`} ]}>
                                    <Text style={[styles.activity, {color: item.category.color}]}>{item.category.name}</Text>
                                </View>
                            ...
                            <View style={styles.ballsContainerQR}>
                                ...
                                <View style={styles.ballsHideContainer}>
                                    {hide === true ? (
                                        ...
                                        <Text style={styles.ballsHideText}>из {item.client_hide_balls !== undefined ? item.client_hide_balls : 0} {getBallsText(item.client_hide_balls)}</Text>
                                    )}
                                </View>
                                <View style={styles.ballsClientContainer}>
                                    ...
                                    />
                                    {hide === true ? (
                                        <Text style={styles.ballsClientText}>{item.client_hide_balls !== undefined ? item.client_hide_balls : 0} {getBallsText(item.client_hide_balls)}</Text>
                                    ) : (
                                        <Text style={styles.ballsClientText}>{item.client_balls !== undefined ? item.client_balls : 0} {getBallsText(item.client_balls)}</Text>
                                    )}
                                </View>
                            </View>
            ...
            { addCompany && (
                <Link href={'/secondary/categories'} style={[styles.view, styles.viewLink, { height: itemHeight }]}>
