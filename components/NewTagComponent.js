    ...
    return (
        <View style={styles.tagContainer}>
            <View style={styles.tagContainerText}>
                <TouchableOpacity onPress={() => {handleTagPress(); !isDropdownTag ? handleSubTagPress(selectedTag) : null}}>
                    <Text style={[styles.tagText, !isDropdownTag ? (oneSelectTag ? {backgroundColor: '#343148', color: '#9A95B2'} : null) : null]}>
                ...
                {isDropdownOpen && isDropdownTag && (
                    <View style={styles.dropdownContainer}>
                        {tag.map((subTag, subIndex) => (
                            subTag !== selectedTag && (
                                <TouchableOpacity key={subIndex} onPress={() => handleSubTagPress(subTag.name)}>
